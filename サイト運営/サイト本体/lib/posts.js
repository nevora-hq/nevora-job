...
function renderChartHtml(chart, toc, debugSlug) {
  ...
  // 以下3種は「ファンデーション崩れタイプ別比較」記事専用の図解タイプ。
  ...
  // 汎用4種(2026-08-08追加)。新規記事はこちらを使う。既存記事のasemoChecklist等
  // 記事専用typeは移行せずそのまま残す。
  ...
  // セルフ診断ウィザード(2026-08-09追加)。詳細は renderDiagnosisHtml のコメント参照。
...
function normalizeFrontmatter(data, slug) {
  return {
    ...
    // 医療免責表示のopt-outフラグ。既定は全記事表示で、"none"の場合のみ非表示にする
    // (ArticleDisclaimer.js参照。カテゴリ別allowlist方式は採用しない)。
...
export async function getPostBySlug(slug) {
  ...
  // (2026-08-08、汎用4type追加時に新設)。上記の既存12記事のif(slug===)分岐とは
  // 独立しており、そちらは変更・削除していない。
 ...
 * カテゴリを無視して全記事から日付順に選んでいた旧実装(2026-08-13修正前)は、
 * 美容サービス記事の次にダイエット記事が出るなど文脈を無視した表示になっていたため、
 * 以下の優先順位で選定する。
 *   1. 同一カテゴリ内で、公開日が自分より新しい記事のうち最も近いもの
 *   2. 同一カテゴリ内に該当がなければ、公開日が自分より古い記事のうち最も近いもの
 *   3. 同一カテゴリに他の記事が1件もなければ、悩みタグ(worry)が1つ以上一致する記事を新着順で選ぶ
 *   4. それも該当がなければnull(「次の記事へ進む」自体を非表示にする)
 */
