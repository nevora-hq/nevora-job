...
const SITE_URL = "https://nevora-job.vercel.app";
...
function buildPinDescription(fm) {
  ...
  const keywordLine = picked.length
    ...
    : "";
  const tail = `くわしくは「NEVORA|美容の総合ガイド」の記事をご覧ください。#${category.replace(
    /[・\s]/g,
    ""
  )}`;
...
function buildPinInPage(opts) {
  ...
  if (figures.length === 0) {
    // 独自レイアウト記事(例: 季節の変わり目肌荒れ対策)は専用クラスのfigureを持つ。
  ...
  // 「NEVORA|美容の総合ガイド」は1行だと帯幅(マスコット分を除く約620px)に
    ...
    for (const { slug, fm } of targets) {
      ...
      try {
        ...
        const result = await page.evaluate(buildPinInPage, {
          ...
          siteName: "NEVORA|美容の総合ガイド",
          domain: "nevora-job.vercel.app",
