...
// ---- 架空の専門家証言(B5)の検出 --------------------------------------
// 「税理士さんに相談してみる」のような一般的な相談の勧めは誤検知になるため対象外とし、
// 「税理士さんに〜と言われた/聞いた」「担当者によると」のような、実在しない専門家の
// 発言を引用する文型のみを検出する(2026-08-11、ヘアアイロン記事の誤検知を確認し修正)。
//
// 職業名は配列で持ち、正規表現をそこから組み立てる(2026-08-28、副業サイトを参照実装として
// 全サイトで共通化)。以前は「美容師さん」「薬剤師さん」を正規表現に直書きしていたため、
// 美容以外のサイトでは定義(.claude/agents/writer.md 等)がそのジャンルの職業に直っているのに
// スクリプト側は美容語彙のまま、という不一致が起きていた。
//
// **他サイトへ展開するときは EXPERTS_BY_SITE の中身だけを差し替える。**
// EXPERTS_COMMON と、その下の組み立てロジック・正規表現はどのサイトでも同一のまま使う。
// 差し替える語は、そのサイトの `.claude/agents/writer.md` の「架空の専門家証言」に
// 書かれている職業を正とする(定義とスクリプトの語彙を一致させるため)。
const EXPERTS_COMMON = [
  "医師",
  "弁護士",
  "税理士",
  "看護師",
...
// このサイト(副業)固有の職業。writer.md の「税理士さんに〜と言われた」「担当者によると」
// に対応し、副業・お金まわりの記事で登場しやすい士業を足している。
const EXPERTS_BY_SITE = [
  "社労士",
  "社会保険労務士",
  "ファイナンシャルプランナー",
  "FP",
...
const RE_B5_FAKE_EXPERT = new RegExp(
  [
    ...
    `(?:${EXPERT_ALT})${EXPERT_HONORIFIC}によると`,
    `(?:${EXPERT_ALT})${EXPERT_HONORIFIC}曰く`,
  ].join("|")
);
